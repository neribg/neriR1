...
El archivo `stack-aamo.yml` del proyecto define cuatro servicios interconectados con sus respectivas configuraciones de puertos, volúmenes y variables de entorno.
...
La segunda prueba de verificación involucró la ejecución del archivo `stack-aamo.yml` que define la arquitectura multi-contenedor del proyecto:

```bash
docker compose -f stack-aamo.yml up -d
```
...
docker compose -f stack-aamo.yml logs
